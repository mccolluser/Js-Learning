...
window.addEventListener("DOMContentLoaded", function () {
  ...
  let more = document.querySelector('.more'),
      ...
      close = document.querySelector('.popup-close'); // Определение браузера пользователя
  ...
  let answer = detectBrowser(); // Выбор анимации
  ...
  persons.addEventListener('change', function (event) {
    ...
    if (restDays.value == '') {
      ...
      animate(total); // totalValue.innerHTML = total;
    }
  });
  ...
  place.addEventListener('change', function () {
    if (restDays.value == '' || personsSum.value == '') {
      ...
      animate(ans); // totalValue.innerHTML = a * this.options[this.selectedIndex].value;
    }
